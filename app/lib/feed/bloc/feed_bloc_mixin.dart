...
mixin FeedBlocMixin on Bloc<FeedEvent, FeedState> {
  ...
  static Future<void> _computeSponsoredBlocks(List<dynamic> args) async {
    ...
    // Upstream always had sponsored posts in remote config, so the loop below
    // indexes into this list without checking it. This product carries no
    // advertising and the list is always empty, which made `nextInt(0)` throw
    // and took the whole feed down with it. Skipping only the insertion keeps
    // the trailing divider and section header that follow.
    while (sponsored.isNotEmpty && tempDataLength > 1) {
